...
export default async function ArticleDetailPage({
  ...
  const related = articles
    ...
    .slice(0, 3);
  const more = (related.length ? related : articles.filter((a) => a.slug !== article.slug)).slice(0, 3);
      ...
      <article className="container-px py-12 md:py-16">
        ...
        <div className="mx-auto mt-10 max-w-3xl space-y-6">
          {article.content.map((p, i) => (
            <p
              key={i}
              className="text-lg leading-relaxed text-muted-foreground first:text-xl first:text-foreground"
            >
              {p}
            </p>
          ))}
